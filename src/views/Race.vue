  ...
  <div v-if="race.id">
    <div v-if="!streamOverlay && !isPopup">
        ...
        <div class="hero-body">
          <div class="container">
            <div class="columns is-vcentered">
              ...
              <div class="column is-narrow has-text-right has-text-left-mobile" v-if="!race.start_time && race.estimated_start_time">
                <h1 class="subtitle is-5">Estimated start: {{ race.estimated_start_time | LocalTimeFilter }} ({{ race.estimated_start_time | FromNowFilter }})</h1>
              </div>
              <div class="column is-narrow is-hidden-mobile">
                <button :onclick="`window.open('/race/${race.id}','popup','width=550,height=900'); return false;`" target="popup" :href="`/race/${race.id}`" class="button is-primary">Popout</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="section">
        <div class="container">
          <!-- Race is empty -->
          <div class = "notification is-dark has-text-centered" v-if="!characters.length > 0">
            <p>Noone has entered the race.</p>
          </div>
          <!-- Participants -->
          <table class="table is-striped is-hoverable" v-if="characters.length > 0">
            <thead>
              <tr>
                ...
                <th class="has-text-centered" v-if="entry_heroes.length > 1">Hero</th>
            ...
            <tbody class="has-no-overflow">
              <tr v-for="character of characters" :key="character.id">
                ...
                <td class="has-text-centered" v-if="entry_heroes.length > 1">
                  <span :class="`is-hidden-touch has-hero ${ character.hero } subtitle is-5`">{{ character.hero | HeroNameFilter }}</span>
                  <span :class="`is-hidden-desktop has-hero ${ character.hero } subtitle is-5`">{{ character.hero }}</span>
                </td>
                ...
                <td class="has-text-right is-narrow is-hidden-mobile">
                  <p class="subtitle is-6">
                    <CharacterRaceStatus :character="character" :start="race.start_time" :finish="race.finish_time" />
                  </p>
                </td>
              </tr>
            </tbody>
        ...
        <div class="container">
          <div class="tile is-ancestor">
            ...
            <div class="tile is-6 is-vertical is-parent">
              <div class="tile is-child box" v-if="pointsChartData.datasets.length">
              ...
              <div class="tile is-child box" v-if="race.description.length">
                ...
                <div :inner-html.prop="race.description | ParagraphsFilter"></div>
              </div>
              <div class="tile is-child box">
                <p class="subtitle is-4">Rules</p>
                <!-- Type -->
                <p v-if="race.finish_conditions_global">
                  <span class="subtitle is-5 has-text-danger">Type:</span> race ends for everybody when any runner fulfils a finish condition.
                </p>
                <p v-if="!race.finish_conditions_global">
                  <span class="subtitle is-5 has-text-danger">Type:</span> race ends for each runner separately when they fulfil a finish condition.
                </p>
                <!-- Conditions -->
                <div v-for="condition of finish_conditions" :key="condition.id">
                  <p v-if="condition.type === 'time' && condition.time_type === 'race'">
                    <span class="subtitle is-5 has-text-danger">Finish:</span> race ends {{ condition.time }} after the start of the race.
                  </p>
                  <p v-if="condition.type === 'time' && condition.time_type === 'character'">
                    <span class="subtitle is-5 has-text-danger">Finish:</span> race ends {{ condition.time }} after character creation.
                  </p>
                  <p v-if="condition.type === 'quest'">
                    <!--<QuestIcon :id="condition.quest_id" />-->
                    <span class="subtitle is-5 has-text-danger">Finish:</span> complete
                    {{ condition.quest_id | QuestShortNameFilter }}
                    in {{ condition.difficulty | DifficultyFilter }}.
                  </p>
                  <p v-if="condition.type === 'stat'">
                    <span class="subtitle is-5 has-text-danger">Finish:</span> reach {{ condition.counter }} {{ condition.stat | StatNameFilter }}.
                  </p>
                </div>
                ...
                <div class="columns is-mobile has-small-margin-top">
                  ...
                  <div class="column">
                    <Icon v-for="hero of entry_heroes" :key="hero" :name="hero" />
                  </div>
                </div>
              </div>
            </div>
    ...
    <div v-if="streamOverlay">
      ...
      <table class="table is-fullwidth is-transparent" v-if="characters.length > 0">
        <thead>
              <tr class="has-light-text-fade">
                <th>
                  <span class="mr-2">
                    <RaceCountdown :tag="false" :start="race.start_time" :finish="race.finish_time" />
                  </span>
                </th>
                <th class="has-text-right is-narrow"><span class="title is-4">area</span></th>
                <th class="has-text-right is-narrow"><span class="title is-4">pts</span></th>
                <th class="has-text-right is-narrow"><span class="title is-4">lvl</span></th>
              </tr>
            </thead>
        <tbody>
          <tr v-for="character of characters" :key="character.id">
    ...
    <div v-if="isPopup">
      ...
      <!-- Participants -->
      <table class="table is-fullwidth is-striped is-hoverable" v-if="characters.length > 0">
        <thead>
          <tr>
            <th class="has-text-centered has-text-fade">#</th>
            <th>Runner</th>
      ...
      <section class="section has-no-padding-top">
        <div class="box">
          ...
          <div v-for="condition of finish_conditions" :key="condition.id">
            <p v-if="condition.type === 'time' && condition.time_type === 'race'">
              <span class="subtitle is-5 has-text-danger">Finish:</span> ends {{ condition.time }} after the start of the race.
            </p>
            <p v-if="condition.type === 'time' && condition.time_type === 'character'">
              <span class="subtitle is-5 has-text-danger">Finish:</span> race ends {{ condition.time }} after character creation.
            </p>
...
export default {
  ...
  computed: {
    ...mapState({
      ...
      positivePoints: state => state.ws.rules.filter(
        rule => rule.context === 'points' && rule.amount > 0
      ),
      negativePoints: state => state.ws.rules.filter(
        rule => rule.context === 'points' && rule.amount < 0
      ),
      ...
      entry_heroes: state => state.ws.race ? state.ws.race.entry_hero.split(',') : [],
  ...
  watch: {
    ...
    lastUpdateTime() {
      ...
      while (this.pointsLogIndex < this.$store.state.ws.pointsLog.length) {
        const { user_id, update_time, value } = this.$store.state.ws.pointsLog[this.pointsLogIndex];
        const character = this.characters.find(character => character.user_id === user_id);

        if (!character) {
          continue;
        }

        let dataset = this.pointsChartData.datasets.find(d => d.label === character.user_name);
